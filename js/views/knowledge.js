...
function filterTabBtnHtml(f, count, active) {
  return `
    <button type="button" class="tab-btn tab-btn-sm tab-btn-stat ${f.cls}${active ? " active" : ""}" data-filter="${f.id}">
...
function refreshKbStats(el, ctx) {
  ...
  el.querySelectorAll("#kb-filter-tabs [data-filter]").forEach((btn) => {
    const numEl = btn.querySelector(".tab-btn-num");
    if (numEl) numEl.textContent = tagStats[btn.dataset.filter] ?? 0;
  });
    ...
    <div class="page kb-page">
      ...
      <div class="kb-sticky source-vocab-sticky">
        ...
        <div class="tabs kb-filter-tabs source-vocab-filter-tabs" id="kb-filter-tabs">
          ${KB_FILTERS.map((f) => filterTabBtnHtml(f, tagStats[f.id] ?? 0, filter === f.id)).join("")}
        </div>
