...
## Сессия 2026-06-17 — UI-полировка База знаний + модал добавления

**Импорт (`import.js`):** убран подзаголовок; кнопка «Загрузить файл» по центру, шире; подпись «srt или txt».

**Тренировка (`training.js`):** убрана подсказка «1 шаг = 10 карточек»; убран заголовок «Последние шаги»; из таблицы истории убрана колонка «Источник».

**База знаний (`knowledge.js`) — крупный рефактор UI:**
- Переключатель **Слова / Выражения** сверху; статистика (всего / выучено / к повторению) по выбранному типу.
- «К повторению» через `getAppStats().activeWords` / `activePhrases` (не `countDue()`).
- Убраны лишние подзаголовки («База», «Слов на изучении: N»).
- Единая строка поиска `#k-search`; кнопка **«Добавить»** при непустом запросе и 0 результатов в текущей секции.
- Модал `#kb-add-modal` в `index.html`: поле слова/выражения, чипы перевода (словарь), Сохранить/Отмена.
- Сохранение по секции: `addWordManual` / `addPhraseManual` (на изучении), `addKnownWordFromImport` / `addKnownPhraseFromImport` (выучено), `addStopWord` (стоп-лист, только слова).
- Inline-формы добавления убраны из `study-words.js`, `study-phrases.js`.

**Открытый баг:** ~~в модале «Добавить» клик ✕…~~ **исправлено** (делегирование в `trans-chips.js`).

**trans-chips (`js/ui/trans-chips.js`):** делегирование на root; bind модала один раз в `ensureKbAddModal`.

---

## Сессия 2026-06-17 (продолжение) — чипы, источники, сериалы/книги

**trans-chips.js:** делегирование событий (fix ✕ в `#kb-add-modal`); bind модала один раз в `ensureKbAddModal`; «Перевод» — `<div>`, не `<label>`.

**Ручное добавление слов:** `word.manual`; кнопка ≡ → «Добавлено вручную» (`sources-modal.js`, `study-words.js`).

**Сериалы / Книги:** убран подзаголовок «Готовность = …»; N/M изучено — только плашка в шапке карточки.

**Cache-bust:** `?v=20260717`.
