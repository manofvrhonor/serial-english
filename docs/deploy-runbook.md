...
1. Timeweb → Облачные серверы → создать: **Ubuntu**, ~4 GB RAM / 50 GB SSD / 2 CPU,
   локация СПб (дешевле), публичный IP.
2. Безопасность — через агента в Cursor (по SSH), промпты:
...
3. Установить Dockploy: на сервере выполнить официальный установщик
